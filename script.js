...
function updateAnalytics() {
    ...
    if (total === 0) {
        ...
        document.getElementById('statDistinction').textContent = '0%';
        return;
    }
    ...
    document.getElementById('statPassRate').textContent = Math.round((passCount / total) * 100) + '%';
    document.getElementById('statDistinction').textContent = Math.round((distCount / total) * 100) + '%';
...
function updateCharts(history) {
    ...
    distributionChartInstance = new Chart(ctx1, {
        type: 'bar',
        data: {
            labels: labels,
            datasets: [{
...
document.getElementById('clearHistoryBtn').addEventListener('click', clearDatabase);
...
console.log('📊 Exam Score Predictor Pro with Database loaded!');
